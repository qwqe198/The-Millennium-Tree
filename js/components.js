...
// data = a function returning the content (actually HTML)
	...
	Vue.component('resource-display', {
		...
		<div style="margin-top: -13px">
			<span v-if="tmp[layer].baseAmount"><br>你有 {{formatWhole(tmp[layer].baseAmount)}} <span v-html="tmp[layer].baseResource"></span></span>
			<span v-if="tmp[layer].passiveGeneration"><br>你正在获得 {{format(tmp[layer].resetGain.times(tmp[layer].passiveGeneration))}} <span v-html="tmp[layer].resource"></span>每秒</span>
			<br><br>
			<span v-if="tmp[layer].showBest">你最多获得{{tmp[layer].resource}}的数量是{{formatWhole(player[layer].best)}}<br></span>
